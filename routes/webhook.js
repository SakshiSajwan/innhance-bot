...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (message.type === 'image') {
      ...
      await sendText(customerPhone, '🔍 Verifying your payment screenshot, please wait...', phoneNumberId);
    ...
    // HANDLER 1: LEGACY TEXT "paid"
    if (/^(paid|payment done|payment complete|pay kar diya|pay ho gaya)/i.test(userMessage)) {
      const booking = await Booking.findOne({ phone: normalizedPhone, status: 'pending' }).sort({ createdAt: -1 });
      if (booking) {
        ...
        const reply = await getSmartReply(customerPhone, hotel._id, customer._id, userMessage, 'No pending booking found.', detectPreferredLanguage(userMessage));
        await sendText(customerPhone, reply, phoneNumberId);
      }
      return;
    }

    // HANDLER 2: GREETING / MENU
    ...
    if (interactiveId === 'menu_offers' || interactiveId === 'menu_checkin' || interactiveId === 'menu_contact' || 
        interactiveId === 'photo_ask' || ['room_standard', 'room_deluxe', 'room_suite'].includes(interactiveId)) {
      // You can keep your original logic for these or simplify
      const reply = await getSmartReply(customerPhone, hotel._id, customer._id, userMessage, null, detectPreferredLanguage(userMessage));
      await sendText(customerPhone, reply, phoneNumberId);
      return;
    }
    ...
    // HANDLER 5: Smart AI + Force Booking Creation
    ...
    let booking = await tryExtractAndSaveBooking(normalizedPhone, hotel._id, customer._id, history);

    const lowerReply = reply.toLowerCase();
    const isBookingSummary = lowerReply.includes('summary') || lowerReply.includes('total') || 
